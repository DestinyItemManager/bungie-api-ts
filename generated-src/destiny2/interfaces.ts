...
export interface DestinyItemSocketState {
  ...
  /**
   * If this socket has a plug set to be checked for plugs (either Character or
   * Profile scoped), this is the hash identifier for that plug set to be checked.
   *
   * Use this to lookup the additional plugs in ProfilePlugSets (
   * DestinyProfileResponse.profilePlugSets) or CharacterPlugSets (
   * DestinyProfileResponse.characterPlugSets). To determine which to check (one or
   * both), check the plugSources property for their respective source flags.
   */
  readonly plugSetHash?: number;
  /**
   * A Flags Enumeration convenience property that indicates the different locations
   * at which this socket may have plugs.
   *
   * Each possible flag indicates somewhere in the Profile/Character data that needs
   * to be checked for valid plugs. See the definition of this enumeration for more
   * details.
   *
   * This enum represents a set of flags - use bitwise operators to check which of
   * these match your value.
