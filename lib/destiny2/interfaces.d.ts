...
export interface DestinyItemComponent {
    ...
    /**
     * Indicates what type of permission is required from the player before an app can
     * dismantle this item.
     */
    readonly dismantlePermission: any;
}
...
export interface DestinyInsertPlugsRequestEntry {
    ...
    /**
     * Plugs are never instanced. So with the hash alone, we should be able to: 1)
     * Infer whether the player actually needs to have the item, or if it's a reusable
     * plug 2) Perform any operation needed to use the Plug, including removing the
     * plug item and running reward sheets.
     */
    readonly plugItemHash: number;
}
export declare const enum DestinySocketResponseCodes {
    None = 0,
